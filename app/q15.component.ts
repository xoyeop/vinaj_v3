...
export class Q15Component {
	...
	constructor(
   	...
   	this.ages.push({ label: '60', value: 60 });
	 	...
	 	this.emails = [];
	 	this.emails.push({ label: 'naver.com', value: 'naver.com' });
	 	this.emails.push({ label: 'gmail.com', value: 'gmail.com' });
	 	this.emails.push({ label: 'nate.com', value: 'nate.com' });
	 	this.emails.push({ label: 'hanmail.net', value: 'hanmail.net' });
	 	this.emails.push({ label: 'daum.net', value: 'daum.net' });
	 	this.emails.push({ label: 'hotmail.com', value: 'hotmail.com' });
	}
	...
	goNext() {
		this.globals.uCity = this.selectedCity;
		this.globals.uAge = this.selectedAge;
		this.globals.uEmail = this.emailID + "@" + this.selectedEmail;
	...
	save() {
		...
		data.push({
			"exp" : this.globals.exp,

			"vBand" : this.globals.vBand,
			"vCup" : this.globals.vCup,
			"vPant" : this.globals.vPant,

			"brafit" : this.globals.brafit,
			
			"bandfit" : this.globals.bandfit,

			"shoulderfit" : this.globals.shoulderfit,

			"vProsN" : this.globals.vProsN,
			"vProsT" : this.globals.vProsT,

			"vConsN" : this.globals.vConsN,
			"vConsT" : this.globals.vConsT,

			"dailyL" : this.globals.dailyL,
			"dailyU" : this.globals.dailyU,

			"fridayL" : this.globals.fridayL,
			"fridayU" : this.globals.fridayU,

			"weekendL" : this.globals.weekendL,
			"weekendU" : this.globals.weekendU,

			"preferC" : this.globals.preferC,
			
			"presentC" : this.globals.presentC,

			"pPad" : this.globals.pPad,

			"way" : this.globals.way,
			"wayIn" : this.globals.wayIn,

			"uAge" : this.globals.uAge,
			"uCity" : this.globals.uCity,
			"uEmail" : this.globals.uEmail
		});
